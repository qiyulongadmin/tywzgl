			<div id="sidebar" class="sidebar                  responsive                    ace-save-state">
				...
				<ul class="nav nav-list">
                    ...
                    <li class="{{substr(Request::getPathinfo(),0,12)=='/admin/conts'?'active open':'' }}">
						<a href="" class="dropdown-toggle">
							<i class="menu-icon fa fa-pencil-square-o"></i>
							<span class="menu-text">
								内容管理
						...
						<ul class="submenu">
                            ...
                            <li class="{{ Request::url() == url('/admin/conts/'.$v->id.'/conts/'.$v->id.'/conts')?'active':''}}{{ Request::url() == url('/admin/conts/'.$v->id.'/conts/'.$v->id.'/conts/create')?'active':''}}{{substr(Request::getPathinfo(),0,31+2*strlen($v->id))=='/admin/conts/'.$v->id.'/conts/'.$v->id.'/conts/edit'?'active':'' }}">
                                @if($v->type==1||$v->type==2)
                                <a href="{{url('admin/conts/'.$v->id.'/conts/'.$v->id.'/conts')}}">
                                @elseif($v->type==3)
                                <a href="{{url('admin/conts/'.$v->id.'/conts/'.$v->id.'/conts')}}">
                                @elseif($v->type==4)
                                <a href="{{url('admin/conts/'.$v->id.'/conts/'.$v->id.'/conts')}}">
                            ...
                            <li class="{{substr(Request::getPathinfo(),0,19+strlen($v->id))=='/admin/conts/'.$v->id.'/conts'?'active open':''}}">
                                ...
                                <ul class="submenu">
                                    ...
                                    @if($v2->up_id==$v->id)
                                    <li class="{{ Request::url()==url('admin/conts/'.$v->id.'/conts/'.$v2->id.'/conts')?'active':'' }}{{ Request::url()==url('admin/conts/'.$v->id.'/conts/create')?'active':'' }}{{substr(Request::getPathinfo(),0,24+strlen($v->id))=='/admin/conts/'.$v->id.'/conts/edit'?'active':'' }}">
                                        <a href="{{url('admin/conts/'.$v->id.'/conts/'.$v2->id.'/conts')}}">
                                            <i class="menu-icon fa fa-caret-right"></i>
                                            {{$v2->name}}
                                        </a>
